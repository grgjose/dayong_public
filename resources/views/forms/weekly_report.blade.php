...
<head>
    ...
    <style>
        /* same styles as daily_report.blade.php */
        body { font-family: Arial, sans-serif; margin: 0; padding: 0; font-size: 10px; }
        .container { width: 80%; margin: 0 auto; padding: 20px; }
        .header, .footer { text-align: center; padding: 10px; }
        .details table, .summary table, .transactions table { width: 100%; border-collapse: collapse; }
        .details th, .details td, .summary th, .summary td, .transactions th, .transactions td { border: 1px solid #ddd; padding: 8px; }
        .transactions th { background-color: #f2f2f2; }
        .summary th, .summary td { text-align: left; }
        .footer { margin-top: 40px; }
        .signature { margin-top: 50px; }
        .signature .name, .signature .signature-field { display: inline-block; width: 45%; text-align: left; }
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
          ...
          <p>WEEKLY REPORT FOR {{ $startDate }} &ndash; {{ $endDate }}</p>
            ...
            <table>
                ...
                <tbody>
                    ...
                    @foreach($results_col as $colr)
                        ...
                        @php
                            $all_col_number_of_accounts += $colr->number_of_accounts;
                            $all_col_total_amount       += $colr->total_amount;
                            $all_col_total_incentives   += $colr->total_incentives;
                            $all_col_total_net          += $colr->total_net;
                            $all_col_total_fidelity     += $colr->total_fidelity;
                        @endphp
                    @endforeach
                    <tr>
                        <th>TOTAL:</th>
            ...
            <table>
                ...
                <tbody>
                    ...
                    @foreach($results_ns as $nsr)
                        ...
                        @php
                            $all_ns_number_of_accounts += $nsr->number_of_accounts;
                            $all_ns_total_amount       += $nsr->total_amount;
                            $all_ns_total_incentives   += $nsr->total_incentives;
                            $all_ns_total_net          += $nsr->total_net;
                            $all_ns_total_fidelity     += $nsr->total_fidelity;
                        @endphp
                    @endforeach
                    <tr>
                        <th>TOTAL:</th>
        ...
        <div class="summary">
            <h2>Summary</h2>
            <table>
                <tr><th>Total Accounts</th><td>{{ $all_col_number_of_accounts + $all_ns_number_of_accounts }}</td></tr>
                <tr><th>Total Gross</th><td>{{ $all_col_total_amount + $all_ns_total_amount }}</td></tr>
                <tr><th>Total Incentives</th><td>{{ $all_col_total_incentives + $all_ns_total_incentives }}</td></tr>
                <tr><th>Total Fid. Bond</th><td>{{ $all_col_total_fidelity + $all_ns_total_fidelity }}</td></tr>
                <tr><th>Total Net Remittance</th><td>{{ $all_col_total_net + $all_ns_total_net }}</td></tr>
                <tr><th>Expenses</th><td></td></tr>
            </table>
        </div>
